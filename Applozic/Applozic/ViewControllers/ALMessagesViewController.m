...
-(NSMutableArray*)arrayMessageAfterSort{
    
    NSMutableArray* sortArray= [NSMutableArray new];
    
    for (int i=0; i<self.mContactsMessageListArray.count; i++) {
        ALMessage * message = self.mContactsMessageListArray[i];
        if(message.groupId != nil) {
            // It's a group message
                [sortArray addObject:message];
        } else if (message.contactIds != nil)  {
            // It's a normal one to one message
            if (!self.onlyShowGroup) {
                [sortArray addObject:message];
...
{
   // return [self arrayMessageAfterSort].count>0?[[self arrayMessageAfterSort] count]:0;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (indexPath.section)
    {
        ...
        case 1:
        {
            ...
            ALMessage *message = (ALMessage *)[self arrayMessageAfterSort][indexPath.row];
...
{    
    ...
    {
        ALMessage * message = [self arrayMessageAfterSort][indexPath.row];
...
-(void)createDetailChatViewController:(NSString *)contactIds
{
    ...
    self.detailChatViewController.firstTimePresent= YES;


    //[self.navigationController pushViewController:self.detailChatViewController animated:YES];
    
    UINavigationController* nv = [[UINavigationController alloc] initWithRootViewController:self.detailChatViewController];
    
    nv.modalTransitionStyle   = UIModalTransitionStyleCrossDissolve;
    
    [[self topMostController] presentViewController:nv animated:YES completion:nil];
...
-(void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (editingStyle == UITableViewCellEditingStyleDelete)
    {
        ...
        ALMessage * alMessageobj = [self arrayMessageAfterSort][indexPath.row];
        
        ALChannelService *channelService = [ALChannelService new];
        
        if([channelService isChannelLeft:[alMessageobj getGroupId]])
        {
            NSArray * filteredArray = [[self arrayMessageAfterSort] filteredArrayUsingPredicate:
        ...
        [ALMessageService deleteMessageThread:alMessageobj.contactIds orChannelKey:[alMessageobj getGroupId]
            ...
            if([alMessageobj getGroupId])
            {
                ...
                theFilteredArray = [[self arrayMessageAfterSort] filteredArrayUsingPredicate:
                                    [NSPredicate predicateWithFormat:@"groupId = %@",[alMessageobj getGroupId]]];
            }
            else
            {
...
-(IBAction)createGroup:(id)sender
{
   ...
   // [self.navigationController pushViewController:groupCreation animated:YES];
    ...
    UINavigationController* nv = [[UINavigationController alloc] initWithRootViewController:groupCreation];
