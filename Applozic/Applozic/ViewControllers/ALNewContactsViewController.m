...
- (void)viewDidLoad
{
    ...
    self.searchBar = [[UISearchBar alloc] initWithFrame:CGRectMake(5 ,y +10 , self.view.frame.size.width - 10, 35)];
    ...
    self.navigationController.navigationBar.barTintColor = [UIColor colorWithRed:86.0/255 green:128.0/255 blue:179.0/255 alpha:1];
    [[UINavigationBar appearance] setTintColor:[UIColor whiteColor]];
    self.navigationController.navigationBar.titleTextAttributes = [NSDictionary dictionaryWithObject:[UIColor whiteColor] forKey:NSForegroundColorAttributeName];
    [self setStatusBarBackgroundColor:[UIColor colorWithRed:86.0/255 green:128.0/255 blue:179.0/255 alpha:1]];
...
{
    float y = self.navigationController.navigationBar.frame.origin.y+self.navigationController.navigationBar.frame.size.height;
    self.searchBar.frame = CGRectMake(5 ,y +10 , self.view.frame.size.width - 10, 35);
}
        ...
        switch (self.groupOrContacts.intValue)
        {
            ...
            case SHOW_GROUP:
            {
                if(self.filteredContactList.count)
                {
                    ...
                    [newContactCell.contactPersonImageView setImage:[ALUtilityClass getImageFromFramworkBundle:@"applozic_group_icon"]];
                    NSURL * imageUrl = [NSURL URLWithString:channel.channelImageURL];
                    if(imageUrl.path.length)
                    {
                        [newContactCell.contactPersonImageView sd_setImageWithURL:imageUrl placeholderImage:[ALUtilityClass getImageFromFramworkBundle:@"applozic_group_icon"] options:SDWebImageRefreshCached];
                    }else{
                        [newContactCell.contactPersonImageView setImage:[ALUtilityClass getImageFromFramworkBundle:@"applozic_group_icon"]];
                                ...
                                andMetaData:nil withCompletion:^(ALChannel *alChannel, NSError *error) {
                                    ...
                                    if(alChannel)
                                    {
                                       ...
                                       */
                                        ...
                                        [self.delegate backAndDismissViewController];
                                    }
                                    else
                                    {
